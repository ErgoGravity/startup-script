...
/** Represents CLI option description.
  ...
  *
  * @param name The name of this option excluding prefix `--` (should not contain whitespaces) (Examples: `app.conf` for [[ConfigOption]] and `dry-run` for [[DryRunOption]])
...
object ConfigOption extends CmdOption(
  "src/main/conf", FilePType,
  "configuration file path relative to the local directory (Example: `--app.conf ergo_tool.json`")
